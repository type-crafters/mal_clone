...
<section id="hero-banner">
    <div id="banner-container" class="w-full flex justify-center max-[639px]:h-80 sm:bg-gradient-to-r sm:from-black sm:via-neutral-700 sm:via-50% sm:to-black">
        <div id="banner-wrapper" class="max-[639px]:w-full max-[639px]:h-full max-[639px]:relative sm:w-2/3 lg:w-1/2 sm:aspect-video sm:h-auto">
          {#if anime.video}
            <!-- svelte-ignore a11y-media-has-caption -->
            <video 
              title={anime.video.title} 
              class="max-[639px]:absolute max-[639px]:inset-0 w-full h-full max-[639px]:object-cover"
              controls
            >
          ...
          {:else}
            ...
            <picture 
              ...
              class="max-[639px]:absolute max-[639px]:inset-0 w-full h-full sm:overflow-hidden"
            >
              {#each anime.image.srcset as source}
                <source media={source.media} srcset={source.src} type={source.mime}/>
              {/each}
              <img 
                src={anime.image.srcset[0].src} 
                alt="fallback" 
